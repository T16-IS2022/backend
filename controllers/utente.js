...
const cancella_account = async (req, res) => {
    ...
    Annuncio.deleteMany({ _id: { $in: user.annunci_pubblicati } }, (err) => {
		if (err)
		  return res.status(500).json({ message: 'Errore durante la cancellazione degli annunci.' });
	});
...
const get_annunci_pubblicati = async (req, res) => {
	...
	return res.status(200).json({ code: 200, message: 'Elenco degli annunci pubblicati ottenuto correttamente', annunci_pubblicati: annunci_pubblicati });
}
...
const get_annunci_salvati = async (req, res) => {
	...
	return res.status(200).json({ code: 200, message: 'Elenco degli annunci salvati ottenuto correttamente.', annunci_salvati: annunci_salvati });
}
...
const get_ricerche_salvate = async (req, res) => {
	...
	return res.status(200).json({ code: 200, message: 'Elenco delle ricerche salvate ottenuto correttamente.', ricerche_salvate: ricerche_salvate });
}
...
const get_chat = async (req, res) => {
	...
	return res.status(200).json({ code: 200, message: 'Elenco delle chat ottenuto correttamente.', lista_chat: lista_chat });
}
...
const modifica_profilo = async (req, res) => {
    ...
    .then(() => {
    	res.status(200).json({ code: 200, message: 'Modifica del profilo riuscita.' });
    })
    .catch((err) => {
